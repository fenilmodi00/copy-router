...
*ai&-exclusive OpenAI-compatible router. Module path / install package remain
`workweave/router`; HTTP override headers still use the `x-weave-*` prefix.*
...
2. **Force check.** Routing intent is read from the `model` field (or
   `/force-model` command, or `x-weave-force-model` header — in that
...
exactly equivalent to `/force-model` or `x-weave-force-model` (same session
pin, same-turn serving, same HTTP 400 on unknown values). Precedence is
`/force-model` command > `model` field > `x-weave-force-model` header; `auto`
...
key rides in an `X-Weave-Router-Key` HTTP header. `--codex --local` and custom
...
For a hosted router, `npx @workweave/router --claude`. Pin with catalog IDs
