...
- Produces: table `router.accounts` (`id varchar(36) PK`, `aiand_user_id varchar(128) NOT NULL`, `aiand_organization_id varchar(128) NOT NULL`, `display_name varchar(255)`, `created_at`, `last_login_at`, `deleted_at`), table `router.account_sessions` (`id uuid PK DEFAULT gen_random_uuid()`, `account_id varchar(36) NOT NULL REFERENCES router.accounts(id) ON DELETE CASCADE`, `token_hash varchar(64) NOT NULL`, `token_prefix varchar(16) NOT NULL`, `token_suffix varchar(16) NOT NULL`, `issued_at`, `expires_at`, `revoked_at`, `last_seen_at`, `ip_at_issue inet`), query functions `UpsertAccount :one`, `GetAccountByAiandUserID :one`, `GetAccountByID :one`, `InsertAccountSession :one`, `GetActiveAccountSessionByHash :one`, `TouchAccountSessionLastSeen :execrows`, `RevokeAccountSessionByID :execrows`, `RevokeAllAccountSessionsForAccount :execrows`, `ListAccountSessionsForAccount :many`.
