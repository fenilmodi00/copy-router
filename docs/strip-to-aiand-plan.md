...
- [x] Edit `internal/translate/` to drop Gemini emit and stream paths unused by Anthropic to OpenAI-compat. **deleted on `strip/cut-gemini-proxy-translate` (base `ae56f994` + this wave).** Removed `FamilyGemini` / `FormatGemini` / `ParseGemini` / `PrepareGemini` / `WireFormatGemini` / `EndpointGeminiGenerate`, proxy `FamilyGemini` dispatch arms, Gemini-only tests, and translate AGENTS/CLAUDE scope to Anthropic ⇄ OpenAI. `ProviderGoogle` name constant kept for hmm/rl/force_model/otel (`FamilyUnknown`). Evidence: `go test ./internal/proxy/ ./internal/translate/ ./internal/api/... ./internal/server/... ./internal/providers/ -count=1` EXIT 0; `go build -o /tmp/router-gemini-cut ./cmd/router` EXIT 0.
...
Tip at this sync: `#28` (`efffe9b`). Residual File leftovers after provider-surface trim: Gemini translate **cut on `strip/cut-gemini-proxy-translate`** (base `ae56f994` + this wave; `go test` packages above EXIT 0), training/analytics package keeps (intentionally unchecked).
