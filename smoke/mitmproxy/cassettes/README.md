...
Recorded router→aiand (OpenAI-compat) interactions for the smoke suite's MITM
proxy (`smoke/mitmproxy/`). Each `<sha256>.json` file is one request/response
pair, keyed by a hash of (method, path, body) — the smoke fixtures are
...
Cassettes go stale when a fixture, scenario, or aiand's own response shape
changes. Refresh them locally:

```bash
AIAND_API_KEY=sk-… SMOKE_PROXY_MODE=record make smoke
