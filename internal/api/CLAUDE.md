...
- `admin/` — operational endpoints: `/health`, `/validate`, dashboard `/v1/*`
...
3. **Decide if dashboard data-plane surface.** The dashboard data-plane routes (`/v1/*` metrics, keys, provider-keys, config, excluded-models, content-capture) mount from a single shared route table in [`../../internal/server/dashboard_routes.go`](../server/dashboard_routes.go) for both `selfhosted` and `selfserve` modes; `managed` skips them. New dashboard data-plane endpoints are added as a row in `dashboardRoutes` there, not as a new route inside a mode block. The login surface (`/account/v1/*` aiand-key) stays in `Register` — it is genuinely mode-specific. Product-surface endpoints (`/v1/*`, `/health`, `/validate`) stay outside the table so they're available in `managed` mode too. **Do not** add a new dashboard `/v1/*` route inside a `Register` mode block — add it as a table row, or it will be missed by the other dashboard mode.
