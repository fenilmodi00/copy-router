...
func TestSubscriptionOnly_OpenAI_ServesOnCodexSub(t *testing.T) {
	...
	svc := proxy.NewService(fr, map[string]providers.Client{providers.ProviderOpenAI: p}, nil, false, nil, nil, false, providers.ProviderOpenAI, "openai/gpt-oss-120b", nil)

	body := `{"model":"openai/gpt-oss-120b","messages":[{"role":"user","content":"hi"}],"max_tokens":100,"stream":true}`
...
func TestSubscriptionOnly_OpenAI_PaidRoute_Refuses402(t *testing.T) {
	...
	svc := proxy.NewService(fr, map[string]providers.Client{providers.ProviderOpenRouter: p}, nil, false, nil, nil, false, providers.ProviderOpenAI, "openai/gpt-oss-120b", nil)
	...
	body := `{"model":"openai/gpt-oss-120b","messages":[{"role":"user","content":"Refactor the auth middleware and add tests."}],"max_tokens":4096,"tools":[{"type":"function","function":{"name":"edit_file","parameters":{"type":"object"}}}]}`
	rec, req := codexSubRequest(t, body)
...
func TestSubscriptionOnly_OpenAI_InfrastructureModelRefusesWithoutDispatch(t *testing.T) {
	...
	assert.Contains(t, fr.capturedReq.ExcludedModels, "openai/gpt-oss-120b",
...
func TestSubscriptionOnly_OpenAI_SubFailure_NoPaidFailover(t *testing.T) {
	...
	body := `{"model":"openai/gpt-oss-120b","messages":[{"role":"user","content":"hi"}],"max_tokens":100}`
	rec, req := codexSubRequest(t, body)
